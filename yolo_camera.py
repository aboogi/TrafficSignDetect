...
def yolo_camera(video_path):
    net.path_class_names = os.path.join(
        os.getcwd(), 'yolo_detect_data', 'classes.names')
    net.path_yolo_weights = os.path.join(
        os.getcwd(), 'yolo_detect_data', 'yolov3_ts_train_8500.weights')
    net.path_model_cnn = os.path.join(
        os.getcwd(), 'yolo_detect_data', 'model_tr.h5')
    net.path_cfg = os.path.join(
        os.getcwd(), 'yolo_detect_data', 'yolov3_ts_test.cfg')
    net.path_data = os.path.join(
        os.getcwd(), 'yolo_detect_data', 'ts_data.data')
    ...
    yolo_net, layers_all, layers_names_out, col = net.load_yolo_network(
        net.path_cfg, net.path_yolo_weights, labels)
    # cv2.dnn.readNetFromTensorflow(net.path_model_cnn, path_csv_tf)
    tf_model = load_model(path_model_tf)
    ...
    while True:
        ...
        output_from_network = net.forward_pass(
            yolo_net, yolo_blob, layers_names_out)
        ...
        res_suppression = net.non_max_suppression(bounding_boxes,
                                                  ...
                                                  net.yolo_threshold)
        img_res, data_ts, contour = net.draw_box_and_labels(res_suppression,
                                                            frame,
                                                            labels,
                                                            class_numbers,
                                                            bounding_boxes,
        ...
        if len(data_ts) > 0:
            ...
            if probability_value > threshold:
                ...
                cv2.putText(
                    frame, f"Type of sign: {ts}", (0, 35), font, 0.75, (0, 0, 255), 2, cv2.LINE_AA)
                cv2.putText(frame, str(round(probability_value*100, 2)) +
                            "%", (60, 75), font, 0.75, (0, 255, 0), 2, cv2.LINE_AA)

        print()
        if len(bounding_boxes) != 0:
            print('Total objects been detected:', len(bounding_boxes))
            print('Number of objects left after non-maximum suppression:', contour - 1)
            if ts != 0:
                print(ts)
...
if __name__ == "__main__":
    video_resource = os.path.join(os.getcwd(), "test_data", "video_roads.mp4")
    yolo_camera(video_resource)
